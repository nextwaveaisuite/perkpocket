...
class SecureAdminPanel {
    ...
    setupEventListeners() {
        ...
        document.getElementById('skipAdminVerification').addEventListener('click', () => {
            this.bypassLogin();
        });
    ...
    initSecurity() {
        ...
        this.logSecurityEvent('admin_panel_accessed');
    ...
    attemptLogin() {
        ...
        if (!this.isCaptchaVerified) {
            this.showNotification('Please complete the security verification first.', 'warning');
            return;
        }

        if (key === 'admin-secret-key-here') {
            this.authenticateUser();
        } else {
            this.logSecurityEvent('admin_login_failed', { key: key.substring(0, 3) + '***' });
            this.showNotification('Invalid admin key. Access denied.', 'error');
        }
    }
    ...
    bypassLogin() {
        ...
        this.authenticateUser();
        this.logSecurityEvent('admin_verification_bypassed');
    }
    ...
    authenticateUser() {
        ...
        this.showNotification('Welcome to PerkPocket Admin Panel', 'success');
    ...
    onCaptchaSuccess(token) {
        this.isCaptchaVerified = true;
        this.logSecurityEvent('admin_captcha_verified', { token: token.substring(0, 10) + '...' });
        this.showNotification('Security verification completed', 'success');
    }
    ...
    switchTab(tabName) {
        ...
        // Reset offers view when switching to offers tab
        if (tabName === 'offers') {
            this.showAdminMarketSelection();
        }

        this.logSecurityEvent('admin_tab_switched', { tab: tabName });
    }

    showAdminMarketSelection() {
        document.getElementById('market-selection-admin').style.display = 'block';
        document.getElementById('categories-admin').style.display = 'none';
        document.getElementById('offers-admin').style.display = 'none';
    }

    selectAdminMarket(market) {
        this.currentMarket = market;
        this.showAdminCategories();
        this.logSecurityEvent('admin_market_selected', { market });
    }

    showAdminCategories() {
        document.getElementById('market-selection-admin').style.display = 'none';
        document.getElementById('categories-admin').style.display = 'block';
        document.getElementById('offers-admin').style.display = 'none';

        const marketName = this.currentMarket === 'au' ? '🇦🇺 Australia' : '🇬🇧 United Kingdom';
        document.getElementById('admin-market-title').textContent = `${marketName} - Categories`;
    }

    selectAdminCategory(category) {
        this.currentCategory = category;
        this.showAdminOffers();
        this.logSecurityEvent('admin_category_selected', { market: this.currentMarket, category });
    }

    showAdminOffers() {
        document.getElementById('market-selection-admin').style.display = 'none';
        document.getElementById('categories-admin').style.display = 'none';
        document.getElementById('offers-admin').style.display = 'block';

        const categoryName = this.getCategoryDisplayName(this.currentCategory);
        const marketName = this.currentMarket === 'au' ? 'Australia' : 'United Kingdom';
        document.getElementById('admin-offers-title').textContent = `${categoryName} - ${marketName}`;
    ...
    loadAdminOffers() {
        ...
        offers.forEach(offer => {
            const offerCard = this.createAdminOfferCard(offer);
            container.appendChild(offerCard);
        });
    }

    createAdminOfferCard(offer) {
        ...
        card.innerHTML = `
            <div class="admin-offer-header">
                <div>
                    <div class="admin-offer-title">${offer.title}</div>
                    <div class="admin-offer-amount">${offer.amount}</div>
                </div>
            </div>
            <div class="admin-offer-meta">${this.currentMarket.toUpperCase()} • ${this.getCategoryDisplayName(this.currentCategory)}</div>
            <div class="admin-offer-description">${offer.description}</div>
            <div class="admin-offer-actions">
                <button class="admin-btn edit" onclick="adminPanel.editOffer('${offer.id}')">✏️ Edit</button>
                <button class="admin-btn test" onclick="adminPanel.testOffer('${offer.id}')">🔗 Test Link</button>
                <button class="admin-btn delete" onclick="adminPanel.deleteOffer('${offer.id}')">🗑️ Delete</button>
            </div>
        `;
    ...
    editOffer(offerId) {
        this.logSecurityEvent('admin_offer_edit', { offerId });
        this.showNotification(`Editing offer: ${offerId}`, 'info');
    ...
    testOffer(offerId) {
        ...
        if (offer) {
            this.logSecurityEvent('admin_offer_test', { offerId, url: offer.url });
            this.showNotification(`Testing link for: ${offer.title}`, 'info');
            // In production, this would open the affiliate link in a new tab
    ...
    deleteOffer(offerId) {
        if (confirm('Are you sure you want to delete this offer?')) {
            this.logSecurityEvent('admin_offer_delete', { offerId });
            this.showNotification(`Offer deleted: ${offerId}`, 'success');
            // In production, this would remove the offer from the database
            this.loadAdminOffers(); // Refresh the view
        }
    }

    loadDashboardData() {
        // In production, this would fetch real data from the backend
        this.logSecurityEvent('admin_dashboard_loaded');
    }
    ...
    getSessionId() {
        if (!this.sessionId) {
            this.sessionId = 'admin-' + Math.random().toString(36).substr(2, 9);
        }
    ...
    monitorActivity() {
        ...
        const resetTimer = () => {
            ...
            inactivityTimer = setTimeout(() => {
                this.logSecurityEvent('admin_session_timeout');
                this.showNotification('Session expired due to inactivity', 'warning');
                // In production, this would log out the admin
            }, inactivityLimit);
        };
...
function selectAdminMarket(market) {
    window.adminPanel.selectAdminMarket(market);
}

function selectAdminCategory(category) {
    window.adminPanel.selectAdminCategory(category);
}

function showAdminMarketSelection() {
    window.adminPanel.showAdminMarketSelection();
}

function showAdminCategories() {
    window.adminPanel.showAdminCategories();
}

// Global hCaptcha callback for admin
function onAdminCaptchaSuccess(token) {
    window.adminPanel.onCaptchaSuccess(token);
}

// Initialize admin panel
document.addEventListener('DOMContentLoaded', () => {
    window.adminPanel = new SecureAdminPanel();
});
